...
def get_1yr_data(ticker):
    url_1yr = f'{stock_api_url}/{ticker}/chart/5y?token={IEX_CLOUD_API_TOKEN}'
...
def daily_volatility(ticker):
    ...
    percent_change = points_change.div(highlows['low'])
...
def highest_volatility(ticker):
    ...
    highest_vol = highlows.nlargest(1,"%")
    return highest_vol.iloc[0,4]

def highest_price(ticker):
    "Returns highest price in a given time period"
    prices = get_daily_highlows(ticker)
    highest_price = prices.nlargest(1,"high")
    return highest_price.iloc[0,1]
